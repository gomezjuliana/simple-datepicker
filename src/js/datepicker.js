...
class Calendar {
   ...
   */
  addEventListeners(){
    document.querySelector('.datepicker__back').addEventListener('click', (e) => this.onBack());
    document.querySelector('.datepicker__forward').addEventListener('click', (e) => this.onForward());
  ...
  buildCalendar(days){
    ...
    const monthLabel = `<h2 class="datepicker__month">${monthsOfTheYear[this.state.month]} ${this.state.year}</h2>`
  ...
  changeDate(type, amount){
    const currentSelection = new Date(`${this.state.month}, ${this.state.currentSelection}, ${this.state.year}`)
    ...
    const month = type == 'subtract'? theDayToday.getMonth(): theDayToday.getMonth() + 1
  ...
  createDaysOfTheMonth(days, months) {
    ...
    const firstDayOfTheMonth = new Date(`${this.state.month} 01 ${this.state.year}`)

    if (firstDayOfTheMonth.getDay() == 0) {
      return `<div class="calendar">${createDaysOfTheWeek(Array(days).fill(1)).join('')}</div>`
    } else {
      const emptyDays = `${createEmptyDivs(Array(firstDayOfTheMonth.getDay()).fill(1)).join('')}`
  ...
  onBack (e, fromKeyboard) {
    const previousMonth = this.state.month == 1 ?
      new Date(`12 01 ${this.state.year - 1}`) : new Date(`${this.state.month - 1} 01 ${this.state.year}`)
  ...
  onForward(e, fromKeyboard) {
    const nextMonth = this.state.month == 12 ?
    new Date(`01 01 ${this.state.year + 1}`) : new Date(`${this.state.month + 1} 01 ${this.state.year}`)
